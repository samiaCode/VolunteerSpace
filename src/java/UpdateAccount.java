...
public class UpdateAccount extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            //3. get a connection
            con = DriverManager.getConnection(databaseURL, userNameDB, password);
        ...
        try {
            String updateStmt = "UPDATE users "
                    + "SET user_name=?, email=?, contact_number=?, date_of_birth=?, address=? "
                    + "WHERE user_id=?";
 ...
 if ("Admin".equals(bean.getRole())) {
    RequestDispatcher rd = request.getRequestDispatcher("Admin_Account.jsp");
    rd.forward(request, response);
} else if ("Volunteer".equals(bean.getRole())) {
    RequestDispatcher rd = request.getRequestDispatcher("Volunteer_AccountSetting.jsp");
    rd.forward(request, response);
} else {
    RequestDispatcher rd = request.getRequestDispatcher("Organizer_Account.jsp");
    rd.forward(request, response);
}
